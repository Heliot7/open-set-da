...
mxArray *process(const mxArray *mximage, const mxArray *mxsbin) {
  float *im = (float*)mxGetData(mximage);
  ...
  int cells[2];
  cells[0] = (int)floor(0.5+((double)dims[0] - sbin - 2)/(double)sbin);
  cells[1] = (int)floor(0.5+((double)dims[1] - sbin - 2)/(double)sbin);
  ...
  float *feat = (float *)mxGetData(mxfeat);
  
  int visible[2];
  visible[0] = cells[0]*sbin;
  visible[1] = cells[1]*sbin;

  for (int x = -sbin/2; x < visible[1]+sbin/2; x++) {
    for (int y = -sbin/2; y < visible[0]+sbin/2 ; y++) {

      int auxX = x + sbin/2 + 1;
      int auxY = y + sbin/2 + 1;
      float *s = im + auxX*dims[0] + auxY;
      ...
      // Store pixel gradient in a histogram of oriented gradients
      // - Size: cellX*cellsY*num_ori
      // - Pick the highest values of 3 channels
      // - Add pixel's ponderate value to the 4 nearest cells
      // Output: For each cell -> Histogram of 18 orientations (hist)
      if (ixp >= 0 && iyp >= 0) {
  ...
  // - norm = matrix of cellX*cellY storing sum of orientations of each cell
  // - sum norms of energies in the same pos/neg orientation
  for (int o = 0; o < 9; o++) {
  ...
  for (int x = 0; x < out[1]; x++) {
    for (int y = 0; y < out[0]; y++) {
      ...
      // (1) Take norm of adjacent cells
      // (2) n = 1/sqrt(4 adjacent norms + 1)
      // This gives the energy that would be in the border of the output cell
      // Issues: if no energy (borders?) then "n" is very high
      ...
      // - Take average of norm + each orientation in the outCell
