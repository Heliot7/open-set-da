...
double fun_l2r_l1l2_svm ( double *w, int w_size, double *alpha, int l, double *diag, int &nSV )
{
	double v = 0;
	nSV = 0;
  int i;
...
double  fun_l2r_l1l2_svm_hinge ( double *w, int w_size, const problem *prob )
{
  double sum = 0.0;
  ...
  for ( int i = 0; i < prob->l; i++ )
  {
    ...
    double deltaTerm = prob->y[i];
    if ( deltaTerm != 1.0 && deltaTerm != -1.0 ) {
      fprintf(stderr, "Label looks strange: %f\n", deltaTerm);
      exit(-1);
    }
	...
	double Cp, double Cn, int solver_type)
{
  ...
  //
  if ( prob->alpha_in != NULL && prob->w_in != NULL )
  {
    // start with previous solution
    for(i=0; i<l; i++)
	  	alpha[i] = prob->alpha_in[i];
  	for(i=0; i<w_size; i++)
	  	w[i] = prob->w_in[i];
  } else {
	...
	while (iter < max_iter)
	{
    // If you want to see some optimization happening, simply uncomment the next lines :)
    //if ( iter % 5 == 0 )
    //{
    //  int nSV = 0;
	  //  info("[%d] objective value = %lf\n", iter, fun_l2r_l1l2_svm ( w, w_size, alpha, l, diag, nSV ) );
    //}
      ...
      // scalar product of w and an example
			feature_node *xi = prob->x[i];
			while(xi->index!= -1)
			{
	...
	info("Objective value = %lf\n",v/2);
  // compute the hinge loss (only for debugging)
	//info("Objective value (hinge) = %lf\n", fun_l2r_l1l2_svm_hinge(w, w_size, prob));
	info("nSV = %d\n",nSV);

  if ( prob->alpha_in != NULL && prob->w_in != NULL )
  {
    // write back the solution
    // this is mostly for storing alpha :)
    for(i=0; i<l; i++)
	  	prob->alpha_in[i] = alpha[i];
  	for(i=0; i<w_size; i++)
	  	prob->w_in[i] = w[i];
  }
...
{
	double eps=param->eps;
  double objval = 0.0; // return value only defined for some solvers
	int pos = 0;
	int neg = 0;
	for(int i=0;i<prob->l;i++)
		if(prob->y[i] > 0)
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			objval = solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			objval = solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL);
			break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
...
model* train(const problem *prob, const parameter *param)
{
  ...
  double objval = 0.0;
	   ...
	   param->solver_type == L2R_L2LOSS_SVR_DUAL)
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		objval = train_one(prob, param, &model_->w[0], 0, 0);
	}
	else
	{
...
#undef NO_REORDERING
    ...
    // standard case: no warm start
    sub_prob.alpha_in = NULL;
    sub_prob.w_in = NULL;
        ...
        for(i=0;i<l;i++)
          sub_prob.y[i] = prob->y[i];
				...
				objval = train_one(&sub_prob, param, &model_->w[0], weighted_C[0], weighted_C[1]);
			}
			else
			{
        ...
        if ( prob->alpha_in != NULL && prob->w_in != NULL )
        {
          sub_prob.alpha_in = Malloc(double, sub_prob.l);
          sub_prob.w_in = Malloc(double, w_size);
        }
          ...
          for(int ii=0;ii<l;ii++)
            sub_prob.y[ii] = (prob->y[ii] == label[i]) ? 1 : -1 ;
#else
					int si = start[i];
					int ei = si+count[i];
          ...
          // storing previous alpha values (take care of the permutation)
          if ( prob->alpha_in != NULL && prob->w_in != NULL)
          {
            for ( int j = 0 ; j < sub_prob.l; j++ )
              sub_prob.alpha_in[j] = prob->alpha_in[ i*sub_prob.l + perm[j] ];
            for ( int k = 0 ; k < w_size; k++ )
              sub_prob.w_in[k] = prob->w_in[ k + w_size*i ];
          }

					objval += train_one(&sub_prob, param, w, weighted_C[i], param->C);

          // alpha_in and w_in will be overwritten with the solution
          if ( prob->alpha_in != NULL && prob->w_in != NULL)
          {
            for ( int j = 0 ; j < sub_prob.l; j++ )
              prob->alpha_in[ i*sub_prob.l + perm[j] ] = sub_prob.alpha_in[j];
            for ( int k = 0 ; k < w_size; k++ )
              prob->w_in[ k + w_size*i ] = sub_prob.w_in[k];
          }
    ...
    if ( sub_prob.alpha_in != NULL )
      free ( sub_prob.alpha_in );
    if ( sub_prob.w_in != NULL )
      free ( sub_prob.w_in );
