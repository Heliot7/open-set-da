	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'G':
				...
				// mexPrintf("near: %f", param.near_preasure);
				param.far_preasure = atof(argv[i]);
				// mexPrintf("far: %f", param.far_preasure);
				break;
			case 'V':
...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	// EXTRA OPEN
	// -> prob.nr_classes = number of classes in problem
	// -> prob.labels = number of instances  per class 
	int nr_classes = 0;
	for(int idx = 0; idx < label_vector_row_num; ++idx)
		if (labels[idx] > nr_classes)
			nr_classes = labels[idx];
	prob.nr_classes = nr_classes;
	// mexPrintf("nr_classes: %d\n", prob.nr_classes);
	if (param.do_open)
	{
		prob.labels = Malloc(int, (ulong)prob.nr_classes);
		for(int idx = 0; idx < prob.nr_classes; ++idx)
			prob.labels[idx] = idx+1;
		//for(int idx = 0; idx < label_vector_row_num; ++idx)
			//prob.labels[(int)labels[idx]-1]++;
		//for(int idx = 0; idx < prob.nr_classes; ++idx)
			//mexPrintf("label %d: %d\n", idx, prob.labels[idx]);
	}
	else
...
// read in a problem (in svmlight format)
int read_problem_dense_w_svm(const mxArray *label_vec, const mxArray *instance_mat)
{
	int i, j, k;
	int elements, max_index, sc, label_vector_row_num;
	double *samples, *labels;

	prob_one_wsvm.x = NULL;
	prob_one_wsvm.y = NULL;
	x_space = NULL;
	...
	elements = 0;
	// the number of instance
	prob_one_wsvm.l = (int)mxGetM(instance_mat);
	label_vector_row_num = (int)mxGetM(label_vec);

	if(label_vector_row_num!=prob_one_wsvm.l)
	{
		mexPrintf("Length of label vector does not match # of instances.\n");
		return -1;
	}
	...
	// EXTRA OPEN
	// -> prob_one_wsvm.nr_classes = number of classes in problem
	// -> prob_one_wsvm.labels = number of instances  per class 
	int nr_classes = 0;
	for(int idx = 0; idx < label_vector_row_num; ++idx)
		if (labels[idx] > nr_classes)
			nr_classes = labels[idx];
	prob_one_wsvm.nr_classes = nr_classes;
	// mexPrintf("nr_classes: %d\n", prob_one_wsvm.nr_classes);
	if (param_one_wsvm.do_open)
	{
		prob_one_wsvm.labels = Malloc(int, (ulong)prob_one_wsvm.nr_classes);
		for(int idx = 0; idx < prob_one_wsvm.nr_classes; ++idx)
			prob_one_wsvm.labels[idx] = idx+1;
		//for(int idx = 0; idx < label_vector_row_num; ++idx)
			//prob_one_wsvm.labels[(int)labels[idx]-1]++;
		//for(int idx = 0; idx < prob_one_wsvm.nr_classes; ++idx)
			//mexPrintf("label %d: %d\n", idx, prob_one_wsvm.labels[idx]);
	}
	else
	...
	if(param_one_wsvm.kernel_type == PRECOMPUTED)
		elements = prob_one_wsvm.l * (sc + 1);
	else
	{
		for(i = 0; i < prob_one_wsvm.l; i++)
		{
			for(k = 0; k < sc; k++)
				if(samples[k * prob_one_wsvm.l + i] != 0)
					elements++;
			// count the '-1' element
			elements++;
		}
	...
	prob_one_wsvm.y = Malloc(double,prob_one_wsvm.l);
	prob_one_wsvm.x = Malloc(struct svm_node *,prob_one_wsvm.l);
	x_space = Malloc(struct svm_node, elements);
	...
	for(i = 0; i < prob_one_wsvm.l; i++)
	{
		...
		prob_one_wsvm.y[i] = labels[i];

		for(k = 0; k < sc; k++)
		{
			if(param_one_wsvm.kernel_type == PRECOMPUTED || samples[k * prob_one_wsvm.l + i] != 0)
			{
				x_space[j].index = k + 1;
				x_space[j].value = samples[k * prob_one_wsvm.l + i];
				j++;
			}
		}
		x_space[j++].index = -1;
...
int read_problem_sparse_w_svm(const mxArray *label_vec, const mxArray *instance_mat)
{
	int i, j, k, low, high;
	mwIndex *ir, *jc;
	int elements, max_index, num_samples, label_vector_row_num;
	...
	// the number of instance
	prob_one_wsvm.l = (int)mxGetN(instance_mat_col);
	label_vector_row_num = (int)mxGetM(label_vec);
	...
	elements = num_samples + prob_one_wsvm.l;
	max_index = (int)mxGetM(instance_mat_col);
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 1 && nrhs < 4)
	{
		...
		if (param.svm_type == ONE_VS_REST_WSVM)
		{
			...
			param.optimize = OPT_BALANCEDRISK; //By default, optimize risk
			if(param.kernel_type == PRECOMPUTED)
				err = read_problem_dense_w_svm(prhs[0], prhs[1]);
			else
				err = read_problem_sparse_w_svm(prhs[0], prhs[1]);
		...
		else
		{
			...
			const char *error_msg;
			// mexPrintf("start training!\n");
			model = svm_train(&prob, &param);
			// mexPrintf("done training, start copy!\n");
			error_msg = model_to_matlab_structure(plhs, nr_feat, model, 0);
			// mexPrintf("copy done!\n");
			if(error_msg)
			...
			if (param.svm_type == ONE_VS_REST_WSVM)
			{
				...
				error_msg = model_to_matlab_structure(plhs, nr_feat, model_one_wsvm, 1);
				if(error_msg)
					mexPrintf("Error: can't convert libsvm model to matrix structure: %s\n", error_msg);
			...
			{
				// mexPrintf("enters here: num: %d\n", nlhs);
				// error_msg = model_to_matlab_structure(plhs, nr_feat, model, 1);
				if(nlhs == 2)
					plhs[1] = plhs[0];
			}
		}
